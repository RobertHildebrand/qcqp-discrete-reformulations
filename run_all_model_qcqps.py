...
home_dir = sys.argv[1]
#home_dir = './All_instances'
#home_dir = '.'


### IMPORTANT: name for run for result summary saving. User-specified so as not to become unwieldy. ###
...
#run_mthds = [BIN2, BIN3, UNIVAR, TNMDT, TDNMDT, NMDT, DNMDT]
run_mthds = [TNMDT, TDNMDT]
# read_mthds = []
...
for L in [1, 2]:
    ...
    process_results = 0
    run_mdl = 1
    #write_models = 1  # Writes all so-far unwritten models, unless remodel_anyway is selected; if so, writes all models.
    remodel_anyway = 1   # Bypass to re-create all models, in case of reruns with caught bugs in the model generation code.
    ...
    mdl_path = rf'{home_dir}/testcases-single'
    res_path = rf'{home_dir}/results'
    orig_path = rf'{home_dir}/original'

    #with open(f'{home_dir}/best_sols.txt', "r") as file:
    #    best_sols = eval(file.read())

    myfpath = rf'{orig_path}/*.lp'
    pp.pprint(myfpath)
    fpaths = glb(myfpath)
    pp.pprint(fpaths)
    for fpath in fpaths:
        ...
        for mthd in mthds:
            ...
            mpath = rf'{mdl_path}/{pname}{mthd_str}.lp'
            rpath = rf'{res_path}/{pname}{mthd_str}'
            if mthd in run_mthds:
                if mthd != DIRECT:
                    ...
                    elif run_mdl or remodel_anyway:
                        # No need to read in if not running and model already exists, and not remodel_anyway.
                        mdl = read(mpath)
            ...
            if mthd in run_mthds and run_mdl:
                if mthd == DIRECT:
                    mdl.setParam('NonConvex', 2)
                mdl.setParam("LogFile", f'{rpath}_{maxTime}s.log')
                mdl.setParam('TimeLimit', maxTime)
                mdl.setParam('OutputFlag', 1)
                mdl.optimize()
                if mdl.SolCount >= 1:
                    mdl.write(f'{rpath}_{maxTime}s.mst')
                    mdl.write(f'{rpath}_{maxTime}s.sol')
    ...
    if process_results:
        ...
        for mthd in mthds:
            ...
            rstr = rf'{home_dir}/{mthd_res_strs[mthd]}'
        ...
        for cls in clss:
            ...
            if n_cls >= 1:
                BB_cls = {mthd_res_strs[mthd]: sum(abs(bnds[mthd][i] - BB[i]) / BB[i] <= 1e-5 for i in Is)
                          for mthd in mthds}
            ...
            rdict = {'n': n_cls, 'BB': BB_cls, 'TO': TO_cls, 'time': tave_cls, 'nsols': nsols_cls}
            rstr = rf'{home_dir}/res_{runstr}_{clsStr[cls]}.txt'
