...
if doStrangeTest:
    fpath = r"testcases/minlp_ac_opf_case3_to_case9/minlp_ac_opf_nesta_case3_lmbd__sad_gurobi_BIN2_L=1_L1=10.lp"
    mdl = read(fpath)
...
try:
    ...
    opath = rf'original/{oname}.lp'

    # print(oname)
    # print(opath)

    # Method and read/run/write settings
    maxTime = 60 * 60 * 2  # Tell Lukas to add a 10 minute to the maximum runtime as a file IO allowance
    maxTime = 60 * 5
    rpath = rf'results/{fname}_{maxTime}s_{("no", "with")[do_callback]}cb'
