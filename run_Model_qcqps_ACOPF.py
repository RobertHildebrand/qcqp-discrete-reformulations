...
read_mthds = [BIN2, BIN3, UNIVAR, TNMDT, TDNMDT]
#run_mthds = [BIN2, BIN3, UNIVAR, NMDT, DNMDT]
run_mthds = [NMDT, DNMDT]
#read_mthds = []
...
process_results = 0
run_mdl = 1
remodel_anyway = 1  # Bypass to re-create all models, in case of reruns with caught bugs.

if not process_results and not run_mdl:
    # Then re-writing the models is all that's left to do
    remodel_anyway = 1
...
for fpath in fpaths:
    ...
    for mthd in mthds:
        ...
        if mthd in run_mthds or remodel_anyway:
            if mthd != DIRECT:
